...
# target
parser.add_argument("-d","--tar_input_dim",type = int, default = 144)
...
test_label = 'datasets/houston/imggt.mat'

seeds = [1330,1337,1236,1226,1235]
...
def get_train_test_loader(Data_Band_Scaler, GroundTruth, class_num, shot_num_per_class):
    ...
    #process label
    houston = GroundTruth.reshape(1905 * 349, 3)
    ...
    ref = gt.reshape(349, 1905)
    data_band_scaler = utils.flip(Data_Band_Scaler)
    groundtruth = utils.flip(ref)
...
class Network(nn.Module):
    def __init__(self):
        ...
        # self.spectral = nn.Sequential(nn.Linear(160, 320),
        #     nn.ReLU(),
        #     nn.Linear(320, 160),
        #     nn.ReLU())
        # self.spatial = nn.Sequential(nn.Linear(160, 320),
        #                               nn.ReLU(),
        #                               nn.Linear(320, 160),
        #                               nn.ReLU())
