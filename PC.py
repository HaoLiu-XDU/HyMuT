...
# target
parser.add_argument("-d","--tar_input_dim",type = int, default = 102)# salinas=204;pc=102,ip=200
...
for iDataSet in range(nDataSet):
    ...
    for episode in range(15000):
        ...
        # target domain few-shot + domain adaptation
            ...
            # print(mask_feature.shape,mask_logits.shape)
            # print(mask_label.shape)
            mask_loss =  crossEntropy(mask_logits.float(), mask_label.long())

            # total_loss = fsl_loss + domain_loss + punish_a*f_loss3
...
for i in range(best_G.shape[0]):
    ...
    for j in range(best_G.shape[1]):
        ...
        if best_G[i][j] == 2:
            hsi_pic[i, j, :] = [219, 174, 86]
        if best_G[i][j] == 3:
            hsi_pic[i, j, :] = [184, 219, 86]
        if best_G[i][j] == 4:
            hsi_pic[i, j, :] = [105, 219, 86]
        if best_G[i][j] == 5:
            hsi_pic[i, j, :] = [86, 219, 147]
        if best_G[i][j] == 6:
            hsi_pic[i, j, :] = [86, 211, 219]
        if best_G[i][j] == 7:
            hsi_pic[i, j, :] = [86, 131, 219]
        if best_G[i][j] == 8:
            hsi_pic[i, j, :] = [121, 86, 219]
        if best_G[i][j] == 9:
            hsi_pic[i, j, :] = [200, 86, 219]
