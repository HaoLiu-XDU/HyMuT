...
def record_experiment_results(train_start, train_end, test_end, OAMean, OAStd, AAMean, AAStd, kMean, kStd, AMean, AStd, output_path):
    # Create a list of result names
    result_names = ["train_end-train_start", "test_end-train_end", "OAMean", "OAStd", "AAMean", "AAStd", "kMean", "kStd", "AMean", "AStd"]

    # Prepare the result values as a list of lists
    result_values = [
    ...
    for i in range(len(result_names)):
        output_string += result_names[i] + ":\n"
        values = result_values[i]
        if isinstance(values, list):
            for val in values:
                output_string += str(val) + "\n"
        else:
            output_string += str(values) + "\n"
        output_string += "\n"
